...
class AudioPlayer {
    ...
    play(soundKey, options) {
        var audioOptions = Utility.merge({}, Sound.defaultOptions, options);

        if ( soundKey === undefined ) return;
        if ( this.audios[soundKey] === undefined ) return;
        if ( Sound.muted && !audioOptions['background'] ) return;

        if ( this.activeAudioMap[soundKey] === undefined ) {
            this.activeAudioMap[soundKey] = new Audio(Config.Sound.soundPath + this.audios[soundKey].source);
        }
        ...
        audio.play().catch(function(e) {
            if ( audioOptions.background ) {
                ...
                Sound.registerUserInteractionListener(audio.play.bind(audio));
            }
        });
    }

    pause(soundKey, options) {
    ...
    setVolume(soundKey, options, callback) {
        ...
        if (options.fade === undefined || options.fade === 0 ) {
            ...
            var fadeStep = 1 / (options.fade / Config.Sound.fadeTimeOut);
            ...
            var fadeTimeOutFunction = function () {
                ...
                if (fadeIn && audio.volume < options.volume || !fadeIn && audio.volume > options.volume) {
                    //still fading
                    audio.fadeTimeOut = setTimeout(fadeTimeOutFunction, Config.Sound.fadeTimeOut);
                } else {
                    //finished fading
                    if ( typeof callback === 'function' ) callback();
                }
...
export const Sound = {
    ...
    init: function() {
        if (Utility.isSafari() || Utility.isIE()) {
        ...
        if (Storage.has('kurve.sound.muted', 'sessionStorage')) {
            this.muted = Storage.get('kurve.sound.muted', 'sessionStorage');
        } else {
            Storage.set('kurve.sound.muted', this.muted, 'sessionStorage');
        }
    ...
    initSuperpowerSounds: function() {
        for (var i in Superpowerconfig.types) {
            if ( !Array.isArray(Superpowerconfig[i].audios) ) continue;

            Superpowerconfig[i].audios.forEach(function(audio) {
    ...
    preloadAudio: function() {
        for (var i in this.audios) {
            new Audio(Config.Sound.soundPath + this.audios[i].source);
        }
    },
    ...
    getAudioPlayer: function() {
        ...
        var player = new AudioPlayer(this.audios);
    ...
    onCreditsCloseClicked: function () {
        // Lightbox import will be added in main.js to avoid circular dependency
        import('./KurveLightbox.js').then(m => m.Lightbox.hide());
    },

    onCreditsClicked: function () {
        // Lightbox import will be added in main.js to avoid circular dependency
        import('./KurveLightbox.js').then(m => m.Lightbox.show(document.getElementById('credits').innerHTML));
    },
