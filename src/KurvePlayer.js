...
export class Player {
    constructor(id, keyLeft, keyRight, keySuperpower) {
        let points = 0;
        let superpower = Factory.getSuperpower(Superpowerconfig.types.NO_SUPERPOWER);
        let superPowerElement = null;
        let isActive = false;
        let color = null;
        let controllerConnected = false;
        let controllerLeftPressed = false;
        let controllerRightPressed = false;
        let controllerSuperpowerPressed = false;
        ...
        this.setControllerInput = function(left, right, superpower) { 
            controllerLeftPressed = left;
            controllerRightPressed = right;
            controllerSuperpowerPressed = superpower || false;
        };
        
        this.getPoints = function() { return points; };
        this.getId = function() { return id; };
        this.getColor = function() { return color === null ? Theming.getThemedValue('players', id) : color };
        ...
        this.isControllerSuperpower = function() { return controllerSuperpowerPressed; };
    }
        ...
        return  '<div id="' + this.getId() + '" class="player inactive ' + this.getId() +'">' +
                    ...
                    '<div class="superpower">' +
                        ...
                        '<div class="superpowerType light">' +
                            '<div class="left" onclick="menuOnPreviousSuperPowerClicked(event, \'' + this.getId() + '\')"><i class="arrow arrow-left"></i></div>' +
                            '<div class="superpowers">' +
                                '<div id="' + this.getId() + '-superpower">' + this.getSuperpower().getLabel() + '</div>' +
                            '</div> ' +
                            '<div class="right" onclick="menuOnNextSuperPowerClicked(event, \'' + this.getId() + '\')"><i class="arrow arrow-right"></i></div>' +
                        '</div> ' +
                    '</div>' +
                    '<div class="clear"></div>' +
                '</div>';
    ...
    renderNumberOfSuperPowers() {
        var superpowers = '';

        for (var i=0; i < Config.Superpower.maxSuperpowers; i++ ) {
